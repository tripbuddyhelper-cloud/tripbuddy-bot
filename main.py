...
# Включаем DEBUG-логирование TeleBot (после import telebot!)
...
@app.get("/")
def home():
    # Health-check страницы деплоя Replit — отдадим валидный 200 и простой HTML
    return "<!doctype html><title>OK</title><h1>OK</h1>", 200

# Webhook endpoint для Telegram:
# Telegram будет POST'ить апдейты на /webhook/<BOT_TOKEN>
...
def is_admin(message: types.Message) -> bool:
    # 1) Если задан ADMIN_GROUP_ID — разрешаем команды внутри этого чата
    if ADMIN_GROUP_ID_INT is not None and message.chat.id == ADMIN_GROUP_ID_INT:
        return True
    # 2) Если задан ADMIN_USERNAME — разрешаем по username
    if ADMIN_USERNAME:
        my = (message.from_user.username or "")
        if ADMIN_USERNAME.startswith("@"):
            adm = ADMIN_USERNAME[1:]
        else:
            adm = ADMIN_USERNAME
        if my.lower() == adm.lower():
            return True
    # 3) Если ничего не задано — разрешаем всем
...
PORT = int(os.getenv("PORT", "5000"))  # в проде Replit сам подставит PORT

if PUBLIC_URL:
    # ---- ПРОД / DEPLOY: режим WEBHOOK ----
    print(f"Starting in WEBHOOK mode. PUBLIC_URL={PUBLIC_URL}")
    # Чистим и выставляем новый вебхук на наш эндпоинт
    try:
        bot.remove_webhook()
    except Exception as e:
    ...
    # Запускаем только Flask — именно он нужен Replit Deploy
    app.run(host="0.0.0.0", port=PORT, debug=False, use_reloader=False)

else:
    # ---- ЛОКАЛЬНО / WORKSPACE: режим POLLING + мини-Flask (для /health) ----
    ...
    # Поднимем Flask в фоне, чтобы / health-check работал и тут
    def _run_web_bg():
        app.run(host="0.0.0.0", port=PORT, debug=False, use_reloader=False)
    Thread(target=_run_web_bg, daemon=True).start()

    # И запустим обычный long polling
